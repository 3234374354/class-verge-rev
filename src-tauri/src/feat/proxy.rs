...
pub fn copy_classes_env() {
    // 从环境变量获取IP地址，默认127.0.0.1
    let classes_verge_rev_ip =
        env::var("classes_VERGE_REV_IP").unwrap_or_else(|_| "127.0.0.1".to_string());

    let app_handle = handle::Handle::global().app_handle().unwrap();
    let port = { Config::verge().latest().verge_mixed_port.unwrap_or(7897) };
    let http_proxy = format!("http://{classes_verge_rev_ip}:{}", port);
    let socks5_proxy = format!("socks5://{classes_verge_rev_ip}:{}", port);
    ...
    let export_text = match env_type.as_str() {
        ...
        _ => {
            log::error!(target: "app", "copy_classes_env: Invalid env type! {env_type}");
            return;
        }
    };
