...
pub struct IclassesTemp(pub Mapping);

impl IclassesTemp {
    pub fn new() -> Self {
        let template = Self::template();
        match dirs::classes_path().and_then(|path| help::read_mapping(&path)) {
            Ok(mut map) => {
    ...
    pub fn save_config(&self) -> Result<()> {
        help::save_yaml(
            &dirs::classes_path()?,
            &self.0,
            Some("# Generated by classes Verge"),
        )
    }
    ...
    pub fn get_client_info(&self) -> classesInfo {
        let config = &self.0;
...
#[test]
fn test_classes_info() {
    fn get_case<T: Into<Value>, D: Into<Value>>(mp: T, ec: D) -> classesInfo {
        let mut map = Mapping::new();
        map.insert("mixed-port".into(), mp.into());
        map.insert("external-controller".into(), ec.into());

        IclassesTemp(IclassesTemp::guard(map)).get_client_info()
    }

    fn get_result<S: Into<String>>(port: u16, server: S) -> classesInfo {
        classesInfo {
            mixed_port: port,
            socks_port: 7898,
            port: 7899,
    ...
    assert_eq!(
        IclassesTemp(IclassesTemp::guard(Mapping::new())).get_client_info(),
...
pub struct Iclasses {
    ...
    pub secret: Option<String>,
    pub dns: Option<IclassesDNS>,
    pub tun: Option<IclassesTUN>,
    pub interface_name: Option<String>,
}

#[derive(Default, Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
#[serde(rename_all = "kebab-case")]
pub struct IclassesTUN {
...
pub struct IclassesDNS {
    ...
    pub fallback_filter: Option<IclassesFallbackFilter>,
    pub nameserver_policy: Option<Vec<String>>,
}

#[derive(Default, Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
#[serde(rename_all = "kebab-case")]
pub struct IclassesFallbackFilter {
